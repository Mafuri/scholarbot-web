...
# ── DB Models (inlined — no separate database.py needed) ──────
...
class User(Base):
    ...
    def to_dict(self, include_password=False):
        d = {"id":self.id,"name":self.name,"email":self.email,
             "degree_level":self.degree_level,"major":self.major,"school":self.school,
             "nationality":self.nationality,"gpa":self.gpa,"financial_need":self.financial_need,
             "languages":self.languages or [],"skills":self.skills or [],
             "extracurriculars":self.extracurriculars or [],"demographic_tags":self.demographic_tags or [],
             "personal_statement":self.personal_statement or "","plan":self.plan,
             "applications_this_month":self.applications_this_month or 0,
             "created_at":self.created_at.isoformat() if self.created_at else None}
        if include_password: d["password_hash"] = self.password_hash
        return d
...
class Application(Base):
    ...
    def to_dict(self):
        return {"id":self.id,"user_id":self.user_id,"opportunity_id":self.opportunity_id,
                "scholarship_name":self.scholarship_name,"opportunity_type":self.opportunity_type,
                "amount_usd":self.amount_usd,"deadline":self.deadline,"url":self.url,
                "status":self.stage or "researching","notes":self.notes,
...
class Package(Base):
    ...
    def to_dict(self):
        ...
        return {"id":self.id,"user_id":self.user_id,"opportunity_id":self.opportunity_id,
                "scholarship":self.scholarship_name,"amount_usd":self.amount_usd,
                "deadline":self.deadline,"days_left":self.days_left,"url":self.url,
...
class RecRequest(Base):
    ...
    def to_dict(self):
        return {"id":self.id,"user_id":self.user_id,"opportunity_name":self.opportunity_name,
                "recommender_name":self.recommender_name,"recommender_email":self.recommender_email,
                "recommender_title":self.recommender_title,"recommender_institution":self.recommender_institution,
                "relationship_desc":self.relationship_desc,"deadline":self.deadline,
                "submission_link":self.submission_link,"drafted_letter":self.drafted_letter or "",
                "briefing_text":self.briefing_text or "","status":self.status or "requested",
...
class Job(Base):
    ...
    def to_dict(self):
        return {"id":self.id,"status":self.status,"result":self.result,"error":self.error,
...
def init_db():
    ...
    try:
        ...
        for table in Base.metadata.sorted_tables:
            ...
            except Exception as te: print(f"[DB] {table.name}: {te}")
...
def _decode_jwt(token):
    try: return jwt.decode(token, _JWT_SECRET, algorithms=[_JWT_ALG]).get("sub")
            ...
            try:
                r = req.post("https://api.anthropic.com/v1/messages",
                    ...
                    json={"model":"claude-haiku-4-5-20251001","max_tokens":1000,
                          "system":s,"messages":[{"role":"user","content":u}]}, timeout=30)
                return r.json()["content"][0]["text"]
            except: return "I am a motivated student committed to excellence and community impact."
        return claude
...
async def get_scholarships(degree_level: Optional[str]=None, field: Optional[str]=None,
    ...
    profile = user.to_dict() if user else {"degree_level": degree_level or "Graduate",
        "nationality":"Kenya","financial_need":False,"gpa":0,"major":""}
    opps = match_opportunities(profile, opp_type="scholarship", min_amount=min_amount or 0)
    if field: opps = [o for o in opps if field.lower() in o.get("name","").lower() or field.lower() in " ".join(o.get("tags",[])).lower()]
    if region: opps = [o for o in opps if any(region.lower() in c.lower() for c in o.get("eligible_countries",[]))]
    return {"scholarships":opps,"count":len(opps),"total_potential_usd":sum(o["amount_usd"] for o in opps)}
...
async def gen_essay(req: dict, bt: BackgroundTasks, user: User = Depends(get_current_user)):
    ...
    opp = next((o for o in load_all_opportunities() if o["id"]==req.get("scholarship_id","")), None)
    if not opp: raise HTTPException(404, "Opportunity not found")
    db = SessionLocal()
    job = Job(id=f"job_{uuid.uuid4().hex[:8]}", user_id=user.id, job_type="essay", status="running")
    db.add(job); db.commit(); jid = job.id; db.close()
    bt.add_task(_essay_job, jid, opp, user.to_dict(), req.get("tone","personal-narrative"), req.get("max_words",400))
...
async def prep_packages(req: dict, bt: BackgroundTasks, user: User = Depends(get_current_user)):
    db = SessionLocal()
    job = Job(id=f"job_{uuid.uuid4().hex[:8]}", user_id=user.id, job_type="packages", status="running")
    db.add(job); db.commit(); jid = job.id; db.close()
...
async def get_pipeline(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"stages":stages,"counts":{k:len(v) for k,v in stages.items()},"total":len(apps),"won_total_usd":won_total}

@app.patch("/api/pipeline/{app_id}/move")
async def move_stage(app_id: str, data: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_outcome(app_id: str, data: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    a = db.query(Application).filter(Application.id==app_id, Application.user_id==user.id).first()
    if not a: raise HTTPException(404, "Not found")
    a.stage = data.get("status", a.stage); a.notes = data.get("notes", a.notes)
...
async def create_rec(req: RecReqCreate, bt: BackgroundTasks,
    ...
    rec = RecRequest(id=f"rec_{uuid.uuid4().hex[:8]}", user_id=user.id,
                     ...
                     submission_link=req.submission_link, status="requested")
...
async def update_rec_status(rid: str, upd: RecStatusUp,
    ...
    if upd.status == "received": rec.received_at = datetime.utcnow()
...
async def interview_qs(scholarship_slug: str, user: User = Depends(get_current_user)):
    ...
    return {"scholarship":scholarship_slug,
            "questions":QUESTION_BANKS.get(scholarship_slug.lower(), QUESTION_BANKS["general"])}
...
async def dashboard(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    upcoming = sorted([{"name":o["name"],"deadline":o["deadline"],
        ...
        for o in matched if 0<=_days_until(o.get("deadline",""))<=60],key=lambda x:x["days_left"])
    return {"user":profile,"scholarships_matched":len(matched),
            ...
            "total_won_usd":sum(a.amount_usd for a in won),"upcoming_deadlines":upcoming[:5]}

@app.get("/api/health")
async def health(db: Session = Depends(get_db)):
    return {"status":"ok","users":db.query(User).count(),"version":"4.0.0",
...
def _rec_letter_job(rec_id, profile, req):
    ...
    try:
        ...
        try:
            letter = llm("Write a professional recommendation letter. Output only the letter.",
                f"Write a 300-word recommendation for {name} for {opp_name}. "
                f"Student: {major} at {school}, GPA {gpa}. "
                f"Recommender: {rec_name}, {rec_title}.")
        except:
            letter = (f"Dear Committee,\n\nI recommend {name} for {opp_name}. "
                     f"As their {rec_title}, I have seen exceptional work.\n\n"
                     f"Sincerely,\n{rec_name}")
        rec.drafted_letter=letter; rec.briefing_text=f"Student: {name}\nOpportunity: {opp_name}"
        rec.updated_at=datetime.utcnow(); db.commit()
    except Exception as e: logger.error("Rec job failed: %s",e)
    finally: db.close()
